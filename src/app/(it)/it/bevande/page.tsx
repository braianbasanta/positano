...
const title = "Vini e birre italiane · Positano · Eixample, Barcellona";
const description =
  "Una selezione di vini e birre italiane da abbinare alla nostra pizza napoletana e alla pasta fresca al Positano, ristorante italiano nell'Eixample, a Barcellona.";

export const metadata: Metadata = {
  title,
  description,
  alternates: alternatesForPage("bebidas", "it"),
  ...socialFor({ title, description, path: "/it/bevande", locale: "it" }),
};

const categories = [...wines, ...beers];

export default function DrinksItPage() {
  return (
    <>
      <JsonLd
        data={breadcrumbJsonLd([
          { name: "Home", path: "/it" },
          { name: "Bevande", path: "/it/bevande" },
        ])}
      />
      <JsonLd data={beveragesJsonLd("it")} />
      <SiteHeader lang="it" />
      <BottleViewerProvider lang="it">
      <main>
        ...
        <section className="relative overflow-hidden bg-ink px-6 pb-20 pt-36 text-center md:pt-44">
          ...
          <div className="relative mx-auto max-w-2xl">
            ...
            <p className="mt-6 font-serif text-lg leading-relaxed text-cream/85 md:text-xl">
              Una selezione italiana da abbinare a ogni piatto. Dal Vesuvio al Salento, vini e birre scelti apposta per la nostra tavola.
            </p>
          </div>
            ...
            <div className="relative mx-auto mt-16 max-w-4xl">
              ...
              <div className="mt-6 grid gap-6 md:grid-cols-2">
                {featuredWines.map((wine) => (
                  <WineFeature key={wine.name} wine={wine} lang="it" />
                ))}
              </div>
            </div>
          ) : null}
        ...
        <div className="bg-cream px-6 py-20 md:py-28">
                ...
                <section id={category.id} className="scroll-mt-24">
                  ...
                  <ul className="mx-auto mt-10 grid max-w-4xl gap-x-14 gap-y-7 md:grid-cols-2">
                    {category.items.map((wine) => (
                      <WineRow key={wine.name} wine={wine} lang="it" />
                    ))}
                  </ul>
                </section>
              </Reveal>
          ...
          <div className="mx-auto mt-20 max-w-5xl">
            <p className="text-center font-serif text-base italic text-ink-soft">
              IVA inclusa. Chiedi al nostro team informazioni sui nostri vini al bicchiere.
            </p>
