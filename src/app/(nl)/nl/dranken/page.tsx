...
const title = "Italiaanse wijnen en bieren · Positano · Eixample BCN";
const description =
  "Een zorgvuldig samengestelde selectie van Italiaanse wijnen en bieren die perfect passen bij onze Napolitaanse pizza’s en verse pasta bij Positano, een Italiaans restaurant in de wijk Eixample in Barcelona.";

export const metadata: Metadata = {
  title,
  description,
  alternates: alternatesForPage("bebidas", "nl"),
  ...socialFor({ title, description, path: "/nl/dranken", locale: "nl" }),
};

const categories = [...wines, ...beers];

export default function DrinksNlPage() {
  return (
    <>
      <JsonLd
        data={breadcrumbJsonLd([
          { name: "Home", path: "/nl" },
          { name: "Drankjes", path: "/nl/dranken" },
        ])}
      />
      <JsonLd data={beveragesJsonLd("nl")} />
      <SiteHeader lang="nl" />
      <BottleViewerProvider lang="nl">
      <main>
        ...
        <section className="relative overflow-hidden bg-ink px-6 pb-20 pt-36 text-center md:pt-44">
          ...
          <div className="relative mx-auto max-w-2xl">
            ...
            <p className="mt-6 font-serif text-lg leading-relaxed text-cream/85 md:text-xl">
              Een Italiaanse selectie die bij elk gerecht past. Van de Vesuvius tot Salento: wijnen en bieren die we speciaal voor onze tafel hebben uitgekozen.
            </p>
          </div>
            ...
            <div className="relative mx-auto mt-16 max-w-4xl">
              ...
              <div className="mt-6 grid gap-6 md:grid-cols-2">
                {featuredWines.map((wine) => (
                  <WineFeature key={wine.name} wine={wine} lang="nl" />
                ))}
              </div>
            </div>
          ) : null}
        ...
        <div className="bg-cream px-6 py-20 md:py-28">
                ...
                <section id={category.id} className="scroll-mt-24">
                  ...
                  <ul className="mx-auto mt-10 grid max-w-4xl gap-x-14 gap-y-7 md:grid-cols-2">
                    {category.items.map((wine) => (
                      <WineRow key={wine.name} wine={wine} lang="nl" />
                    ))}
                  </ul>
                </section>
              </Reveal>
          ...
          <div className="mx-auto mt-20 max-w-5xl">
            <p className="text-center font-serif text-base italic text-ink-soft">
              Inclusief btw. Vraag ons team naar onze wijnen per glas.
            </p>
